...
let handler = async (m, {
    ...
    let name = await conn.getName(who)
        ...
        if (!/webp|image|video|gif|viewOnce/g.test(mime)) return m.reply(`قم بالاشارة للصورة التي تريد تحويلها لانيم ثم اكتب\n*${usedPrefix + command}*`)
        ...
        m.reply(wait)
        if (out) {
        conn.sendFile(m.chat, out, 'result', "النتيجة ♥ ", m)
        } else {
        throw eror
        }
